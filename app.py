...
model = keras.models.load_model("C:/Users/Lenovo/OneDrive/Desktop/FinalMajor_Proj-codes/fussirecipe_rnn.h5")
data = pd.read_csv('fussirecipe.csv')
data = data.rename_axis('recipe_id').reset_index()

image_path = 'C:/Users/Lenovo/OneDrive/Desktop/FinalMajor_Proj-codes/recfussi_img/'
...
def recommend_recipes_by_image_similarity(recipe_id, num_recommendations=5):
    ...
    similar_recipe_indices = sorted(range(len(target_similarity_scores)), key=lambda i: target_similarity_scores[i],
                                    reverse=True)[1:]
...
def recommend_recipe():
    if request.method == 'POST':
        ...
        if recipe_model == "CNN":
            ...
            return render_template('predict.html', recommended_recipe=my_dict, instruction=instruction,
                                   recipe_name=recipe_name, model=recipe_model)
        else:
            ...
            print(rec)
            recommended_recipes = recommend_recipes_by_image_similarity(10, num_recommendations=5)

            return render_template('predict.html', recommended_recipe=recommended_recipe, recipe_name=recipe_name,
                                   model=recipe_model)
        ...
        print("Modified Recipe Name:", modified_recipe_name)
        print(rec)
        recommended_recipe = recommend_recipes_by_image_similarity(10, num_recommendations=5)

        return render_template('predict.html', recommended_recipe=recommended_recipe, recipe_name=recipe_name)


def predict_recipe(recipe_name, model, max_sequence_length):
    recommended_recipe = generate_text(recipe_name, next_words=100, model=model,
